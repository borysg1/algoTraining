...
public class RangeSearchVisualizer {
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            System.out.println("Rect: (xmin," + rect.xmin() + " ymin," + rect.ymin() + " xmax," + rect.xmax() + " ymax)" + rect.ymax());
            ...
            long redTimeStart = System.currentTimeMillis();
            // draw the range search results for brute-force data structure in red
            StdDraw.setPenRadius(.03);
            StdDraw.setPenColor(StdDraw.RED);
            for (Point2D p : brute.range(rect))
                p.draw();
            long redTimeStop = System.currentTimeMillis();

            System.out.println("RED diff " + (redTimeStop - redTimeStart));

            long blueTimeStart = System.currentTimeMillis();
            // draw the range search results for kd-tree in blue
            StdDraw.setPenRadius(.02);
            StdDraw.setPenColor(StdDraw.BLUE);
            for (Point2D p : kdtree.range(rect))
                p.draw();
            long blueTimeStop = System.currentTimeMillis();

            System.out.println("BLUE diff " + (blueTimeStop - blueTimeStart));
